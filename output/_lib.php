...
/**
  DB
*/
//function dbopen($dbfn, $folder="../phpliteadmin"){
function dbopen($dbfn, $folder="."){
  try {
    $pdo = new PDO('sqlite:'.$folder.'/'.$dbfn);
...
function showtable_withdeledit($pdo, $table,$dbfn=DBFN){
  $tb = tbl($pdo,$table);
  aryinscol($tb, "<a href='_delete.php?table={$table}&id={\$id}&dbfn={$dbfn}'>Delete</a>","delete");
  aryinscol($tb, "<a href='#{$table}{\$id}' onclick='jqedit(\"{$table}\",{\$id},\"{$dbfn}\");'>Edit</a>","edit");
  table( $tb );
}
function table($ret){
  if (count($ret)==0){
...
function jquery(){
  ...
  echo '<script>function jqadd(table,dbfn){
  $("#add_"+table).load("_jqadd.php?table="+table+"&dbfn="+dbfn);
  }
  function jqedit(table,id,dbfn){
  $("#add_"+table).load("_jqedit.php?table="+table+"&id="+id+"&dbfn="+dbfn);
  }</script>';
  $jquery_enabled = true;
}
function modtable($db, $table){
    jqaddform($table,DBFN);
}
...
function jqaddform($table,$dbfn){
  ...
  echo "<button onclick=\"jqadd('{$table}','{$dbfn}')\">Add {$table}</button>";
  echo "</div>";
}
function br($num=1){
...
function endsWith($base,$findlet){
  $baselen = strlen($base);
  $findletlen = strlen($findlet);
  $pos = strpos($base,$findlet);
  if ($pos === ($baselen - $findletlen)) return true;
  return false;
}
function startsWith($base,$findlet){
  $pos = strpos($base,$findlet);
  return ($pos === 0);
}
